...
class CRUDObjectHandle(CRUDBase[ObjectHandleORM, ObjectHandleCreate, None]):
    @staticmethod
    def resolve_handled_object(db: Session, handle: ObjectHandleORM) -> Union[ActionORM,
                                                                              ActionTargetORM,
        ...
        for target_id in crud_dict.keys():
            if getattr(handle, target_id):
                return crud_dict[target_id].read(db, getattr(handle, target_id))
