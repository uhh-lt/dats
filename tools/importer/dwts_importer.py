...
parser.add_argument('--input_dir', type=str, help='Path to directory containing .txt or image files to be imported',
                    required=True, dest='input_dir')
parser.add_argument('--backend_url', type=str, help='URL of the dwts backend api', default='http://localhost:5500/',
                    dest='backend_url')
parser.add_argument('--project_name', type=str, help='Name of the project to import to', default='import',
                    dest='project_name')
parser.add_argument('--project_description', type=str, help='Description of the project to create (only if new project is created)', default='',
                    dest='project_description')
parser.add_argument('--tag_name', type=str, help='Name of the tag that is automatically applied to all documents',
                    default='test', dest='tag_name')
parser.add_argument('--tag_description', type=str,
                    help='Description of the tag that is automatically applied to all documents',
                    default='', dest='tag_description')
parser.add_argument('--is_json', help='Set if the input_dir contains JSON files',
                    default=False, dest='is_json', action='store_true')
...
if tag is None:
    tag = api.create_tag(title=args.tag_name,
