...
class IncelsisSpider(SpiderBase):
    ...
    def __init__(self, page=5, max_pages=1000, *args, **kwargs):
        ...
        self.start_urls = [f'https://incels.is/forums/inceldom-discussion.2/page-{x}' for x in range(int(page), int(max_pages) + 1)]

    def parse(self, response, **kwargs):

        # if on the discussions page
        if response.url.startswith("https://incels.is/forums/inceldom-discussion.2/page-"):
            # get a list of all discussion links
            discussions = response.css('div.structItem-cell.structItem-cell--main > div.structItem-title > a:last-child::attr(href)').getall()
        ...
        if response.url.startswith("https://incels.is/threads/"):
            ...
            title = response.css('#top > div.p-body > div > div.p-body-header > div.p-title > h1::text').getall()
            title = title[0] if title and len(title) > 0 else None
            authors = response.css('div.message-userDetails > h4.message-name > span.username > span::text').getall()
            comments = response.css('div.message-userContent > article.message-body').getall()
