...
class IncelsnetSpider(SpiderBase):
    ...
    def __init__(self, page=2, pages=35, *args, **kwargs):
        ...
        self.start_urls = [f'{self.discussions_url}{x}' for x in range(int(page), int(pages) + 1)]
    ...
    def parse(self, response, **kwargs):
        ...
        if response.url.startswith(self.threads_url):
            ...
            title = response.css('div.p-body > div > div.p-body-header > div.p-title > h1::text').getall()
            title = title[0] if title and len(title) > 0 else None
            authors = response.css('article > div > div.message-cell.message-cell--user > section > h4 > a::text').getall()
            comments = response.css('article > div > div.message-cell.message-cell--main > div > div > div > article > div.bbWrapper').getall()
