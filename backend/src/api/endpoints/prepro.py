...
@router.get("/project/{proj_id}/status", tags=tags,
...
async def get_project_prepro_status(*,
                                    proj_id: int,
                                    db: Session = Depends(get_db_session)) -> PreProProjectStatus:
    ...
    finished_sdocs = crud_sdoc.count_by_project(db=db, proj_id=proj_id, only_finished=True)

    return PreProProjectStatus(project_id=proj_id,
