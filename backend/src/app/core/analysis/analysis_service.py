...
from app.core.data.dto.analysis import AnalysisQueryParameters, CodeOccurrence, \
    CodeFrequencies
...
class AnalysisService(metaclass=SingletonMeta):
    ...
    def analyse_code_frequencies(self,
                                 query_params: AnalysisQueryParameters) -> Dict[str, Any]:  # todo: should return CodeFrequencies
        with self.sqls.db_session() as db:
            query = db.query(SourceDocumentORM, CodeORM, SpanTextORM.text, func.count().label("count")) \
            ...
            query = query.filter(and_(SourceDocumentORM.project_id == query_params.proj_id,
                                      AnnotationDocumentORM.user_id.in_(query_params.user_ids)))
            query = query.group_by(SourceDocumentORM, CodeORM, SpanTextORM.id)
            res = query.all()

            project_codes = [CodeRead.from_orm(code) for code in
                             crud_project.read(db=db, id=query_params.proj_id).codes]
            code_occurrences = [CodeOccurrence(sdoc=SourceDocumentRead.from_orm(x[0]),
        ...
        code_statistics = {code.id: {"code": code,
                                     "count": 0,
                                     "aggregated_count": 0,
                                     "occurrences": [],
                                     "children": []} for code in project_codes}
        ...
        for code in project_codes:
            if code.parent_code_id is not None:
                code_statistics[code.parent_code_id]["aggregated_count"] += code_statistics[code.id]["count"]
        ...
        for code in project_codes:
            if code.parent_code_id is None:
                ...
                code_statistics[code.parent_code_id]["children"].append(code_statistics[code.id])
