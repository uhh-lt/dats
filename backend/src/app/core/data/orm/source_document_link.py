...
class SourceDocumentLinkORM(ORMBase):
    ...
    parent_source_document_id = Column(
        Integer, ForeignKey("sourcedocument.id", ondelete="CASCADE"), index=True
    )
    parent_source_document: "SourceDocumentORM" = relationship(
        "SourceDocumentORM",
        viewonly=True,
        foreign_keys="sourcedocumentlink.c.parent_source_document_id",
    )

    linked_source_document_id = Column(
        Integer,
        ForeignKey("sourcedocument.id", ondelete="CASCADE"),
        nullable=True,
        index=True,
    )
    linked_source_document: "SourceDocumentORM" = relationship(
        "SourceDocumentORM",
        viewonly=True,
