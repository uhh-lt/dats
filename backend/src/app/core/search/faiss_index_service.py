...
class FaissIndexDoesNotExistError(Exception):
    def __init__(self, proj_id: int, index_type: IndexType):
        super().__init__(
            f"There exists no FAISS Index of type {index_type} for Project {proj_id}"
        )
...
class FaissIndexEmptyError(Exception):
    def __init__(self, proj_id: int, index_type: IndexType):
        super().__init__(
            f"The FAISS Index of type {index_type} for Project {proj_id} is empty!"
        )
...
class FaissIndexService(metaclass=SingletonMeta):
    ...
    def __create_index(self) -> IndexIDMap:
        return faiss.index_factory(
            self._index_dimension, self._index_factory_string, self._index_search_metric
        )

    def __persist_index(self, index: IndexIDMap, proj_id: int, index_type: IndexType):
        index_fn = self._get_index_path_for_project(
            proj_id=proj_id, index_type=index_type
        )
        logger.debug(
            f"Persisting {index_type} index for Project {proj_id} at {index_fn}..."
        )
        ...
        logger.debug(
            f"Persisted {index_type} index for Project {proj_id} at {index_fn}!"
        )

    def index_exists(
        self, proj_id: int, index_type: IndexType, raise_if_not_exists: bool = False
    ) -> bool:
        exists = self._get_index_path_for_project(
            proj_id=proj_id, index_type=index_type
        ).exists()
    ...
    def add_to_index(
        ...
        logger.debug(
            f"Added {len(embeddings)} embeddings to {index_type} index of Project {proj_id}!"
        )
        self.__persist_index(index=index, proj_id=proj_id, index_type=index_type)

    def remove_from_index(
        self, embedding_ids: np.ndarray, proj_id: int, index_type: IndexType
    ) -> None:
        logger.debug(
        ...
        logger.debug(
            f"Removed {len(embedding_ids)} embeddings to {index_type} index of Project {proj_id}!"
        )
        self.__persist_index(index=index, proj_id=proj_id, index_type=index_type)

    def create_or_load_index_for_project(
        self, proj_id: int, index_type: IndexType
    ) -> IndexIDMap:
        index = self._index_in_memory_cache.get((proj_id, index_type), None)
        if index is not None:
            logger.debug(
                f"Returning {index_type} index for Project {proj_id} from memory!"
            )
            return index
        ...
        if not index_fn.exists():
            if not index_fn.parent.exists():
                index_fn.parent.mkdir(parents=True, exist_ok=False)
            logger.debug(
                f"Creating {index_type} index for Project {proj_id} at {index_fn}!"
            )
            index = self.__create_index()
            faiss.write_index(index, str(index_fn))
        else:
            logger.debug(
                f"Loading {index_type} index for Project {proj_id} from {index_fn} in memory!"
            )
    ...
    def search_index(
        self, proj_id: int, index_type: IndexType, query: np.ndarray, top_k: int = 10
    ) -> Dict[int, float]:
        if query.ndim == 1:
