...
class NoSuchSourceDocumentInElasticSearchError(Exception):
    def __init__(self, proj_id: int, sdoc_id: int):
        super().__init__((f"There exists no SourceDocument with ID={sdoc_id} in Project {proj_id}"
...
class NoSuchMemoInElasticSearchError(Exception):
    def __init__(self, proj_id: int, memo_id: int):
        super().__init__((f"There exists no Memo with ID={memo_id} in Project {proj_id}"
                          " in the respective ElasticSearch Index!"))
...
class ElasticSearchService(metaclass=SingletonMeta):
    ...
    def create_project_indices(self,
        ...
        self.__create_index(index=self.__get_index_name(proj_id=proj_id, index_type='doc'),
        ...
        self.__create_index(index=self.__get_index_name(proj_id=proj_id, index_type='memo'),
    ...
    def remove_project_indices(self,
                               *, proj_id: int) -> None:
        self.__delete_index(index=self.__get_index_name(proj_id=proj_id, index_type='doc'))
        self.__delete_index(index=self.__get_index_name(proj_id=proj_id, index_type='memo'))
    ...
    def add_document_to_index(self,
        ...
        res = self.__client.index(index=self.__get_index_name(proj_id=proj_id, index_type='doc'),
        ...
        logger.debug(
            f"Added Document '{esdoc.filename}' with ID '{res['_id']}' to Index '{self.__get_index_name(proj_id=proj_id, index_type='doc')}'!")
        return res['_id']
    ...
    def get_esdocs_by_sdoc_ids(self,
        ...
        if not fields.union(self.doc_index_fields):
            raise NoSuchFieldInIndexError(index=self.__get_index_name(proj_id=proj_id, index_type='doc'), fields=fields,
                                          index_fields=self.doc_index_fields)
        results = self.__client.mget(index=self.__get_index_name(proj_id=proj_id, index_type='doc'),
                                     _source=list(fields),
        ...
        for res in results["docs"]:
            if not res["found"]:
                raise NoSuchSourceDocumentInElasticSearchError(proj_id=proj_id, sdoc_id=res["_id"])
    ...
    def get_esdoc_by_sdoc_id(self,
        ...
        if not fields.union(self.doc_index_fields):
            raise NoSuchFieldInIndexError(index=self.__get_index_name(proj_id=proj_id, index_type='doc'), fields=fields,
                                          index_fields=self.doc_index_fields)
        res = self.__client.get(index=self.__get_index_name(proj_id=proj_id, index_type='doc'),
                                id=str(sdoc_id),
                                _source=list(fields))
        if not res["found"]:
            raise NoSuchSourceDocumentInElasticSearchError(proj_id=proj_id, sdoc_id=sdoc_id)
    ...
    def delete_document_from_index(self,
                                   proj_id: int,
                                   sdoc_id: int) -> None:
        self.__client.delete(index=self.__get_index_name(proj_id=proj_id, index_type='doc'), id=str(sdoc_id))
        logger.info(
    ...
    def add_memo_to_index(self,
        ...
        res = self.__client.index(index=self.__get_index_name(proj_id=proj_id, index_type='memo'),
    ...
    def get_esmemo_by_memo_id(self,
        ...
        if not fields.union(self.doc_index_fields):
            raise NoSuchFieldInIndexError(index=self.__get_index_name(proj_id=proj_id, index_type='doc'), fields=fields,
                                          index_fields=self.doc_index_fields)
        res = self.__client.get(index=self.__get_index_name(proj_id=proj_id, index_type='memo'),
                                id=str(memo_id),
                                _source=list(fields))
        if not res["found"]:
    ...
    def delete_memo_from_index(self,
                               *,
                               proj_id: int,
                               memo_id: int) -> None:
        self.__client.delete(index=self.__get_index_name(proj_id=proj_id, index_type='memo'), id=str(memo_id))
        logger.info(
    ...
    def __search_sdocs(self,
        ...
        esdocs = [ElasticSearchDocumentHit(sdoc_id=doc["_id"], score=doc["_score"]) for doc in res.hits.hits]
        ...
        return PaginatedElasticSearchDocumentHits(hits=esdocs,
                                                  ...
                                                  has_more=has_more,
                                                  current_page_offset=skip if skip is not None else 0,
                                                  next_page_offset=(skip + limit) if skip is not None and limit is not None and has_more else 0)
    ...
    def __search_memos(self,
                       *,
                       ...
                       limit: Optional[int] = None,
                       skip: Optional[int] = None) -> PaginatedMemoSearchResults:
    ...
    def search_memos_by_exact_title(self,
        ...
        return self.__search_memos(proj_id=proj_id, query={
            "bool": {
                "must": [
                    {
                        "term": {
