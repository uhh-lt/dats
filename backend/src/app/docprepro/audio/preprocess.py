...
from app.docprepro.audio.generate_and_import_transcript_file import generate_and_import_transcript_file_
...
def import_audio_document(doc_file_path: Path, project_id: int, mime_type: str) -> List[PreProAudioDoc]:
...
@celery_worker.task(acks_late=True, autoretry_for=(Exception,), retry_kwargs={'max_retries': 5, 'countdown': 5})
def generate_and_import_transcript_file(ppads: List[PreProAudioDoc]) -> List[PreProAudioDoc]:
