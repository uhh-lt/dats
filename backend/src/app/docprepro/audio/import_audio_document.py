...
def import_audio_document_(doc_file_path: Path, project_id: int, mime_type: str) -> List[PreProAudioDoc]:
    # persist in db
    dst, sdoc_db_obj = persist_as_sdoc(doc_file_path, project_id)

    # create ppad
    ppad = PreProAudioDoc(project_id=sdoc_db_obj.project_id,
    ...
    # update sdoc status 
    update_sdoc_status(sdoc_id=ppad.sdoc_id, sdoc_status=SDocStatus.import_audio_document)
