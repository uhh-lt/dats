...
def generate_and_import_transcript_file_(ppads: List[PreProAudioDoc]) -> List[PreProTextDoc]:
    pptds = []
    for ppad in tqdm(ppads, desc="Create Transcript file and import file to database..."):
        update_sdoc_status(sdoc_id=ppad.uncompressed_sdoc_id, sdoc_status=SDocStatus.create_transcript_file) #TODO: sdoc_id is not ideal
        ...
        try:
            ...
            logger.error(f'IOError while creating file {str(path_transcription)}. Errormsg: {e}')

        # Import transcript textfile to database and create pptd
        pptd = import_text_document_(doc_file_path=path_transcription, project_id=project_id, mime_type=mime_type)
        
        update_sdoc_status(sdoc_id=ppad.uncompressed_sdoc_id, sdoc_status=SDocStatus.finished) #TODO: sdoc_id is not ideal
...
def create_sdoc_link_pptd_(pptd: PreProTextDoc, parent_source_document_id: int) -> PreProTextDoc:
    ...
    with sql.db_session() as db:
        ...
        update_sdoc_status(sdoc_id=pptd.sdoc_id, sdoc_status=SDocStatus.create_sdoc_links_from_text)
    return pptd

# This is a copy from text.import_text_document. Only difference is that Tika is removed and a single object is returned
def import_text_document_(doc_file_path: Path, project_id: int, mime_type: str) -> PreProTextDoc:
    # persist in db
