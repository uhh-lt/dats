...
def create_ppad_from_ppvd_(ppvds: List[PreProVideoDoc]) -> List[PreProAudioDoc]:
    ...
    fake_ppads = [PreProAudioDoc(audio_dst=ppvd.video_dst,
                                project_id=ppvd.project_id,
                                mime_type=ppvd.mime_type, #TODO: This is not good.
                                sdoc_id=ppvd.sdoc_id,
                                metadata=ppvd.metadata) #TODO: This also. 
                  for ppvd in ppvds]
    for fake_ppad in fake_ppads:
        update_sdoc_status(sdoc_id=fake_ppad.sdoc_id, sdoc_status=SDocStatus.create_ppad_from_ppvd)
