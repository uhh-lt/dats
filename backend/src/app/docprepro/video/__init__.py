...
from app.docprepro.text import store_document_in_elasticsearch, finish_preprocessing, store_span_annotations_in_db, \
    generate_span_annotations, resolve_sdoc_links, clean_html, extract_text_from_html_and_create_source_mapping, \
    detect_language, create_sdoc_links_from_html, add_custom_html_tags, store_metadata_in_db, index_text_document_in_faiss
...
generate_and_import_transcript_file = "app.docprepro.audio.preprocess.generate_and_import_transcript_file"


def video_document_preprocessing_apply_async(doc_file_path: Path, project_id: int, mime_type: str) -> Any:
    video_document_preprocessing = (
            Signature(import_video_document, kwargs={"doc_file_path": doc_file_path,
...
def video_document_preprocessing_without_import_apply_async(ppvds: List[PreProVideoDoc]) -> Any:
    video_document_preprocessing = (
