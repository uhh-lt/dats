...
@celery_worker.task(acks_late=True, autoretry_for=(Exception,), retry_kwargs={'max_retries': 5, 'countdown': 5})
def find_similar_images(proj_id: int, query: Union[str, int], top_k: int = 10) -> Dict[int, float]:
    return find_similar_images_(proj_id, query, top_k)


@celery_worker.task(acks_late=True, autoretry_for=(Exception,), retry_kwargs={'max_retries': 5, 'countdown': 5})
def find_similar_sentences(proj_id: int, query: Union[str, int], top_k: int = 10) -> Dict[int, float]:
