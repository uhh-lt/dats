...
index_text_document_in_faiss = "app.docprepro.simsearch.preprocess.index_text_document_in_faiss"
index_image_document_in_faiss = "app.docprepro.simsearch.preprocess.index_image_document_in_faiss"
...
def index_text_documents_apply_async(pptds: List[PreProTextDoc]) -> Any:
    return Signature(index_text_document_in_faiss, kwargs={"pptds": pptds}).apply_async()


def index_image_documents_apply_async(ppids: List[PreProTextDoc]) -> Any:
    return Signature(index_image_document_in_faiss, kwargs={"ppids": ppids}).apply_async()
