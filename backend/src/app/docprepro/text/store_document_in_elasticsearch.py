...
def store_document_in_elasticsearch_(pptds: List[PreProTextDoc]) -> List[PreProTextDoc]:
    ...
    if len(pptds) <= BULK_THRESHOLD:
        for esdoc in tqdm(esdocs, desc="Adding documents to ElasticSearch... "):
            ...
            update_sdoc_status(sdoc_id=esdoc.sdoc_id, sdoc_status=SDocStatus.store_document_in_elasticsearch)
    else:
        ...
        # Flo: update sdoc status
        for pptd in pptds:
            update_sdoc_status(sdoc_id=pptd.sdoc_id, sdoc_status=SDocStatus.store_document_in_elasticsearch)
