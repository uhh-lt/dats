...
def create_document_content_html_file_via_tika(
    filepath: Path, sdoc_db_obj: SourceDocumentORM
) -> Tuple[Path, SourceDocumentORM]:
    logger.info(
        f"Extracting html content via Tika from {filepath.name} for SourceDocument {sdoc_db_obj.id}..."
    )
    if filepath.suffix not in TIKA_SUPPORTED_FILE_EXTENSIONS:
        raise NotImplementedError(
            f"File Extension {filepath.suffix} are not supported!"
        )
    ...
    logger.info(
        f"Created html file with content from {filepath.name} for SourceDocument {sdoc_db_obj.id}!"
    )
...
def import_text_document_(
    ...
    if filepath.suffix in TIKA_SUPPORTED_FILE_EXTENSIONS:
        filepath, sdoc_db_obj = create_document_content_html_file_via_tika(
            filepath=filepath, sdoc_db_obj=sdoc_db_obj
        )
        mime_type = "text/html"
