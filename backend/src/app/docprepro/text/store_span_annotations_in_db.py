...
def store_span_annotations_in_db_(pptds: List[PreProTextDoc]) -> List[PreProTextDoc]:
    with sql.db_session() as db:
        ...
        for pptd in tqdm(pptds, desc="Persisting Automatic SpanAnnotations... "):
            ...
            adoc_db = crud_adoc.read_by_sdoc_and_user(db=db, sdoc_id=pptd.sdoc_id, user_id=SYSTEM_USER_ID, raise_error=False)
            if not adoc_db:
            ...
            for code in pptd.spans.keys():
                ...
                if not db_code:
                    ...
                    create_dto = CodeCreate(name=code_name,
                ...
                create_dtos = [SpanAnnotationCreate(begin=aspan.start,
                                                    end=aspan.end,
                                                    current_code_id=ccid,
                                                    annotation_document_id=adoc_db.id,
                                                    span_text=aspan.text,
                                                    begin_token=aspan.start_token,
                                                    end_token=aspan.end_token) for aspan in pptd.spans[code]]
                try:
                    crud_span_anno.create_multi(db, create_dtos=create_dtos)
                except OperationalError as e:
                    logger.error(f"Cannot store SpanAnnotations of SourceDocument {adoc_db.source_document_id}: {e}")
