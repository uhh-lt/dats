...
@celery_worker.task(acks_late=True, autoretry_for=(Exception,), retry_kwargs={'max_retries': 5, 'countdown': 5})
def extract_text_from_html_and_create_source_mapping(pptds: List[PreProTextDoc]) -> List[PreProTextDoc]:
...
@celery_worker.task(acks_late=True, autoretry_for=(Exception,), retry_kwargs={'max_retries': 5, 'countdown': 5})
def import_text_document(doc_file_path: Path, project_id: int, mime_type: str) -> List[PreProTextDoc]:
