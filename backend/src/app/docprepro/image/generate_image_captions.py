...
def generate_image_captions_(
    ...
    with tqdm(total=len(ppids), desc="Generating automatic image captions...") as pbar:
        ...
        pixel_values = image_captioning_feature_extractor(
        ...
        output_ids = image_captioning_model.generate(
            pixel_values, max_length=max_caption_length, num_beams=num_beams
        )

        captions = image_captioning_tokenizer.batch_decode(
        ...
        with sql.db_session() as db:
            for ppid, cap in zip(ppids, captions):
                ...
                update_sdoc_status(
                    sdoc_id=ppid.sdoc_id, sdoc_status=SDocStatus.generate_image_captions
                )
                pbar.update(1)
