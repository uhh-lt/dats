...
class SpacyService(metaclass=SingletonMeta):
    def __new__(cls, *args, **kwargs):
        cls.sqlr: SQLRepo = SQLRepo()
        cls.ray: RayRepo = RayRepo()
        return super(SpacyService, cls).__new__(cls)

    def handle_spacy_job(self, payload: SpacyJobInput) -> None:
        # 1. call spacy in ray
        spacy_output = self.ray.spacy_pipline(
            SpacyInput(
                text=payload.text,
        ...
        with self.sqlr.db_session() as db:
            ...
            sdoc_data = self.extract_tok_sent_data(spacy_output)

            # keywords
            # if payload does not have keywords:
            keywords = self.extract_keywords(
                payload, spacy_output, keyword_project_metadata.id
            )

            # word frequencies
            word_frequencies = self.extract_word_frequencies(payload, spacy_output)

            # span annotations
            span_annotations = self.extract_span_annotations(
    ...
    def extract_word_frequencies(
        self, payload: SpacyJobInput, spacy_output: SpacyPipelineOutput
