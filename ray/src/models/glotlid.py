...
@serve.deployment(**build_ray_model_deployment_config("glotlid"))
class GlotLIDModel:
    def __init__(
        self,
        ...
        # note that fasttext only supports CPU
        self.model = fasttext.load_model(model_path)

    def identify_language(self, input: GlotLIDInput) -> GlotLIDOutput:
        labels, confs = self.model.predict(input.text, input.top_k)
        labels = list(map(lambda x: x.replace("__label__", ""), labels))
        detected_languages = [
            DetectedLanguage(
                lang_code=lang_code,
                lang_name=GLOT_LID_LANG_LABEL_TO_NAME.get(
                    lang_code.split("_")[0], "UNKNOWN"
                ),
                confidence=conf,
            )
            for lang_code, conf in zip(labels, confs)
        ]
        best_match = detected_languages[0]
        return GlotLIDOutput(
            best_match=best_match, detected_languages=detected_languages
        )


# https://github.com/cisnlp/GlotLID/blob/main/assets/inference/language_names.json
GLOT_LID_LANG_LABEL_TO_NAME = {
    "aaa": "Ghotuo",
    "aab": "Alumu-Tesu",
    "aac": "Ari",
    "aad": "Amal",
