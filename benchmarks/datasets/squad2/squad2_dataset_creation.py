...
def create_squad2_dataset(split: str, output_path: Path) -> None:
    ...
    for index, sample in enumerate(dataset):
        ...
        is_impossible = bool(sample.get("is_impossible", False))
    ...
    print("SQuAD v2 dataset creation completed.")
    print(f"Rows: {len(df)} -> {output_path}")
    print(f"Answerable rows: {int(df['is_answerable'].sum())} / {len(df)}")
    print(f"Unanswerable rows: {int((~df['is_answerable']).sum())} / {len(df)}")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Download and preprocess SQuAD v2 dataset for extractive QA benchmarks"
    )
    parser.add_argument("--split", default="validation", help="HuggingFace split")
    parser.add_argument(
        "--output",
        default="datasets/squad2/validation.parquet",
...
def main() -> None:
    ...
    create_squad2_dataset(split=args.split, output_path=output_path)
