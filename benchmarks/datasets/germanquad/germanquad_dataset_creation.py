...
def create_germanquad_dataset(split: str, output_path: Path) -> None:
    dataset = load_dataset("deepset/germanquad", split=split)
    ...
    print("GermanQuAD dataset creation completed.")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Download and preprocess GermanQuAD dataset for extractive QA benchmarks"
    )
    parser.add_argument("--split", default="test", help="HuggingFace split")
    parser.add_argument(
        "--output",
        default="datasets/germanquad/test.parquet",
...
def main() -> None:
    ...
    create_germanquad_dataset(split=args.split, output_path=output_path)
