...
function SelectionStatistics({ projectId, aspectId }: SelectionStatisticsProps) {
  // global client state
  ...
  const pinnedStatistics = useAppSelector((state) => state.atlas.pinnedStatistics);
  const handlePinStatistics = useCallback(
    (stat: string) => {
      dispatch(AtlasActions.onPinStatistics(stat));
    },
    [dispatch],
  );
  const handleUnpinStatistics = useCallback(
    (stat: string) => {
      dispatch(AtlasActions.onUnpinStatistics(stat));
    },
    [dispatch],
  );
