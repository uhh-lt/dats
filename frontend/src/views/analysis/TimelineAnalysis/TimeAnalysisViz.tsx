...
function TimelineAnalysisViz({ chartData, concepts }: TimelineAnalysisVizProps) {
  // redux
  const provenanceDate = useAppSelector((state) => state.analysis.provenanceDate);
  const provenanceConcept = useAppSelector((state) => state.analysis.provenanceConcept);
  const dispatch = useAppDispatch();

  // event handlers
  const handleClick = (data: any, concept: TimelineAnalysisConcept) => {
    dispatch(AnalysisActions.setProvenanceDate(data.date));
    dispatch(AnalysisActions.setProvenanceConcept(concept.name));
  };
